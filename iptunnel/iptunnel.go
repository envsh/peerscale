...
import (
	...
	_ "github.com/KarpelesLab/pktkit"
	// _ "github.com/KarpelesLab/pktkit"
)
...
const (
	...
	udpTunnelProto = "udpiptunnel/1.0"
	bufSize       = 3072
	udpBufSize    = 65535
)
...
// "/d2hub/iptunnel/1.09339" → port 9339), it overrides tunnelPort.
...
func handleTunnel(s network.Stream) {
	...
	if err != nil {
		log.Printf("[iptunnel] dial %s: %v", addr, err)
		s.Close()
		return
	}
	...
	go func() {
		...
		defer connClose()
		buf := make([]byte, bufSize)
		mr := msgio.NewReaderSize(s, udpBufSize)  // 限制最大包长，防攻击
		for {
			s.SetReadDeadline(time.Now().Add(5 * time.Minute))
			// 读端（stream 上读回完整 IP 包）
			pkt, err := mr.ReadMsg()              // 一次返回一个完整包
			n, rerr := len(pkt), err
			if n > 0 {
	...
	log.Printf("[iptunnel] conn=%d closed: sent=%d recv=%d dur=%s", seq, localSent, localRecv, dur.Round(time.Millisecond))
}
...
func handleUDPTunnel(s network.Stream) {
	...
	start := time.Now()
	peerid := s.Conn().RemotePeer().ShortString()
	log.Printf("[iptunnel] udp conn=%d protocol=%s newed: %v\n", seq, s.Protocol(), peerid)
	...
	if err != nil {
		log.Printf("[iptunnel] udp dial %s: %v", addr, err)
		s.Close()
		return
	}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				log.Printf("[iptunnel] udp read hdr err %s %s: %v", peerid, s.Protocol(), err)
				return
			}
	...
	wg.Wait()
	dur := time.Since(start)
	log.Printf("[iptunnel] udp conn=%d closed: sent=%d recv=%d dur=%s", seq, localSent, localRecv, dur.Round(time.Millisecond))
}

// todo Dial("p2x", peerID)

// peerID format: 123xxx[:port]
// if want a net.Conn, just wrap like this: &iptunnel.P2PConn{stm}
func DialUDP(peerID string, ctx ...context.Context) (*p2pUdpConn, error) {
	...
	uconn.holder.Store(newStreamHolder(stm))
	return uconn, nil
}

type streamHolder struct {
	s network.Stream
	mr msgio.ReadCloser   // 读：每流一个，随流创建
	mw msgio.WriteCloser  // 写：每流一个，随流创建
}

func newStreamHolder(s network.Stream) *streamHolder {
	return &streamHolder{
		s:  s,
		mr: msgio.NewReaderSize(s, udpBufSize), // 65535 限长防攻击
		mw: msgio.NewWriter(s),                 // 4字节BE长度前缀 + 包
	}
}
...
func (udp *p2pUdpConn) implRead(buf []byte) (int, error) {
	...
	buf2, err := stm.ReadMsg()
	if err != nil {
		log.Println(err, udp.peerShort(), udp.protoWithPort)
		return 0, err
	}
	if len(buf) < len(buf2) {
		stm.ReleaseMsg(buf2) // 先归还再 panic，避免泄漏
		panic("buf too small")
	}
	copy(buf, buf2) // check cap(buf)>=lenght
	stm.ReleaseMsg(buf2) // 归还并复用[]byte,防止分配新的[]byte,不归还也可
	return len(buf2), err
}
...
//
//	client := iptunnel.NewHttpClient("12D3KooW...")
	...
	return &http.Client{
		Transport: &http.Transport{
			...
			DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
				...
				log.Println("iptunnel sending CONNECT", addr, peerID, "...")
				...
				if !bytes.Contains(buf.Bytes(), []byte("200")) {
					log.Println("iptunnel:", string(buf.Bytes()))
					stream.Close()
...
func (s *DriftServer) newUDPSession(remoteAddr net.Addr) *udpSession {
	...
	if err != nil {
		log.Printf("[iptunnel] udp-session dial %s: %v", peerhum, err)
		return nil
	}
...
func (s *DriftServer) handleP2x(conn net.Conn) {
	...
	log.Printf("[iptunnel] pre-conn: %s %s %s", peerid.ShortString(), preConnType, localAddr)
	...
	log.Printf("[iptunnel] drift dial %s: %v (open=%s) %s %s", peerid.ShortString(), err, openDur.Round(time.Millisecond), rdproto, preConnType)
	if err != nil {
		return
	}
	...
	wg.Wait()
	dur := time.Since(start)
	log.Printf("[iptunnel] drift closed: %s peer=%s recv=%d sent=%d open=%s dur=%s%s", remoteAddr, peerid.ShortString(), localRecv, localSent, openDur.Round(time.Millisecond), dur.Round(time.Millisecond), connType)
}
